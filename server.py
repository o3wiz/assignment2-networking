...
def read_form_socket_until_term(sock: socket.socket, term_bytes: bytes, secs_timeout: float, chunk_size: int) -> tuple[bytes, ReceiveStatus]:
    buffer = b""
    ...
    if not buffer and status != ReceiveStatus.VALID:
...
class Server:
    ...
    def __get_http_request(self, client_socket: socket.socket) -> tuple[bytes, ReceiveStatus]:
    ...
    def __get_response(self, payload: bytes) -> tuple[bytes, bool]:
        ...
        path = http_request.path if http_request.path else "index.html"
        if "redirect" == path:  # if the client sent the path "redirect"
        ...
        actual_path = f"{self.SERVER_FILES_FOLDER}/{path}"
        if not path_exists(actual_path):  # the file does not exist
        ...
        content = get_binary_file_content(actual_path)
